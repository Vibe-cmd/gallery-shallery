  ...
  createMesh() {
    ...
    if (this.onPhotoClick) {
      ...
      canvas.addEventListener('click', (e) => {
        ...
        // Simple hit test - check if click is near this plane's position
        ...
        if (Math.abs(x - planeX) < planeWidth && Math.abs(y - planeY) < planeHeight) {
          this.onPhotoClick!(this.photo);
        }
      });
    }
  }
...
interface AppConfig {
  items?: { image: string; text: string; photo: Photo }[];
  bend?: number;
  textColor?: string;
  borderRadius?: number;
  font?: string;
...
class App {
  ...
  planeGeometry!: Plane;
  medias: Media[] = [];
  mediasImages: { image: string; text: string; photo: Photo }[] = [];
  ...
  constructor(
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(items, bend, textColor, borderRadius, font, onPhotoClick);
    this.update();
  ...
  createMedias(
    items: { image: string; text: string; photo: Photo }[] | undefined,
    bend: number = 1,
    textColor: string,
    borderRadius: number,
    font: string,
...
export const CircularGallery = ({
  ...
  }, [photos, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, onPhotoClick]);
